...
Точная зависимость Pack от FPF пока остаётся на `d7a7123`: манифест изменяется
параллельной работой. Перевод `reliedOnEditionRef`, манифеста и записи текущести
на `56440a9` должен быть выполнен одним согласованным изменением после её
завершения. Эта проверка не подменяет атомарное обновление pin.
